...
def get_youtube_data(api_key: str, channel_ids: list[str]) -> list[dict[str: Any]]:
    '''Получение данных о видео и каналах с помощью API YouTube'''
    youtube = build('youtube', 'v3', developerKey=api_key)
    data = []
    videos_data = []
    next_page_token = None
    for channel_id in channel_ids:
        channel_data = youtube.channels().list(id=channel_id, part='snippet,statistics').execute()
        while True:
            response = youtube.search().list(channelId=channel_id, part='id,snippet', type='video',
...
def create_database(db_name, params) -> None:
    """Создает новую базу данных для сохранения данных о каналах и видео"""
...
def save_data_to_database(data: list[dict[str, Any]],db_name: str, params: dict) -> None:
    '''Сохранение данных о каналах и видео в БД'''
    conn = psycopg2.connect(dbname=db_name, **params)
